...
const urlProList =
  "http://localhost:3000/products?&_limit=9&_sort=id&_order=desc";
const urlProSale =
  "http://localhost:3000/products?&_limit=6&_sort=price&_order=asc";
    ...
    .then((data) => {
      ...
      data.forEach((element) => {
        ...
        <li class="list-group-item">
            <div class="row">
                <div class="col-sm-4"><a href="detail.html?id=${
                  element.id
                }"><img
                style="width:80px;"
                ...
                <div class="col-sm-8">${
                  element.name
                } <br><br><b>${numberWithCommas(element.price)} VNĐ</b></div>
            </div>
        </li>
          `;
      });
      ...
      data.forEach((element) => {
        strContent += `
            ...
            <a href="detail.html?id=${
              element.id
            }"><span class="chi-tiet">CHI TIẾT</span></a>

            <a href="detail.html?id=${element.id}"><img
                    src="../public/uploads/product/${
                      element.image
                    }" alt=""><span
  ...
  fetch(urlDetailPro)
    ...
    .then((data) => {
      var strContent = `
            ...
            <div class="spchitiet">
                <img src="../public/uploads/product/${
                  data.image
                }" alt="" style="width:100%;">
            </div>
        </div>
        <div class="col-md-5">
            <div class="thongtinsp">
                <ul>
                    ...
                    <li style = "font-size:30px;">${numberWithCommas(
                      data.price
                    )} VNĐ</li>
                    <li style="margin-top: 15px;"><span style="line-height: 25px;font-size: 15px;text-align: justify;">${
                      data.detail
                    }</span></li>
                    ...
                    <li class="soluong1" style="margin-top: 27px;">
                      ...
                      <button onclick="addCart(${
                        data.id
                      })" type="submit" style="margin-top:-5px;" class="btn btn-dark">THÊM VÀO GIỎ HÀNG</button> 
                    </li>
                    <li style="margin-top: 25px;">-----------------------------------------------------------------</li>
                    <li>Mã: # ${data.id}</li>
                    -----------------------------------------------------------------
                    <li>Tag: #Áo đẹp, #thời trang giới trẻ </li>
                </ul>
            </div>
        </div>
        <div class="col-md-3">
...
async function addCart(id_product) {
  ...
  if (existed == -1) {
    let product = await fetch(`${urlProCart}/${id_product}`).then((res) =>
      res.json()
    );
    product.quantity = 1;
    cart.push(product);
...
function showCart() {
  ...
  cart.forEach((data) => {
    ...
    strContent += `
                <tr>
                    ...
                    <td><input min = 1 class="form-control" type="number" onblur="updateCart(${
                      data.id
                    })" id="quantity-${data.id}" value ="${data.quantity}"  style="width:50px;"></td>
                    <td>${numberWithCommas(data.price * data.quantity)}</td>
                    <td><a onclick="deleteCart(${
                      data.id
                    })" style = "color:black;"><i class="fa fa-times-circle" aria-hidden="true"></i></a></td>
                </tr>
        `;
  });
  ...
  strTotal = `
        ...
        <tr>
            <td>Tổng</td>
            <td style = "text-align:right;"><b>${numberWithCommas(
              total
            )} VNĐ</b></td>
        </tr>
        <tr>
...
function checkAfterCheckOut(total) {
  if (total === 0) {
    alert(
      "GIỎ HÀNG CỦA BẠN CHƯA CÓ SẢN PHẨM NÀO. VUI LÒNG TIẾN HÀNH MUA HÀNG <3!"
    );
...
<tr>
    <td colspan="2"><i>Quý khách vui lòng kiểm tra lại thông tin giao hàng và thông tin đơn hàng để tiến hành đặt hàng. Quý khách có thể tra cứu tình trạng đơn hàng tại JACK LANE.com. Chúc quý khách ngày mới tốt lành !</i></td>
</tr>
<tr>
...
function checkOut() {
  ...
  customer_phone_number = document.querySelector(
